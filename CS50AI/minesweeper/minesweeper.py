...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        """
        self.knowledge.append(self.get_new_sentence(cell, count))
        """
            4) mark any additional cells as safe or as mines
               if it can be concluded based on the AI's knowledge base
        """
        self.mark_additional()
        """
            5) add any new sentences to the AI's knowledge base
               if they can be inferred from existing knowledge
        """
        knowledge_copy = deepcopy(self.knowledge)
        shallow_copy = knowledge_copy.copy()
        for s1 in knowledge_copy:
            shallow_copy.remove(s1)
            for s2 in shallow_copy:
    ...
    def get_new_sentence(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                # Add unknown cells, reduce count with known mines
                if 0 <= i < self.height and 0 <= j < self.width:
    ...
    def make_safe_move(self):
        ...
        return safe_cells.pop() if safe_cells else None
